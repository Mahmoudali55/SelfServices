...
Widget buildRequestList(
  ...
  List<RequestDynamicCountModel> dynamicRequests = const [],
}) {
  final all = <CombinedRequest>[
    ...requests.map((r) => CombinedRequest.static(r)),
    ...dynamicRequests.map((d) => CombinedRequest.dynamic(d)),
  ];
          ...
          onTap: () async {
            if (item.isStatic) {
              final shouldRefresh = await Navigator.pushNamed(
                ...
                arguments: {'type': item.staticItem!.type},
              );
              if (shouldRefresh == true) {
                context.read<NotifictionCubit>().getReqCount(
